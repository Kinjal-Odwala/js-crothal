...
ii.Class({
	...
	Definition: {
			...
			if (me.reportFilters.length > 0) {
				...
				for (var index = 0; index < me.reportFilters.length; index++) {
					...
					if (me.reportFilters[index].referenceTableName == "") {
						...
						RowData += "<td align='left' colspan='2'>" + me.populateDropDown(me.reportFilters[index].referenceTableName, me.reportFilters[index].title, 'filter', 0) + "</td>";
					RowData += "</tr>"
				}
			}
				...
				if ($("#" + me.reportFilters[index].title).val() != "") {
					if ($("#" + me.reportFilters[index].title).val() != "0") {
						...
						if (me.reportFilters[index].validation.toLowerCase() == "bit") {
							...
							if ($("#" + me.reportFilters[index].title).val() != "" && ($("#sel" + me.reportFilters[index].title).val() == 1 || $("#sel" + me.reportFilters[index].title).val() == undefined)) {
								me.filter += " And (" + me.reportFilters[index].tableName + "." + me.reportFilters[index].title + " = '" + $("#" + me.reportFilters[index].title).val() + "')";
							}
							else if ($("#" + me.reportFilters[index].title).val() != "" && $("#sel" + me.reportFilters[index].title).val() == 2) {
								me.filter += " And (" + me.reportFilters[index].tableName + "." + me.reportFilters[index].title + " <= '" + $("#" + me.reportFilters[index].title).val() + "')";
							}
							else if ($("#" + me.reportFilters[index].title).val() != "" && $("#sel" + me.reportFilters[index].title).val() == 3) {
								me.filter += " And (" + me.reportFilters[index].tableName + "." + me.reportFilters[index].title + " >= '" + $("#" + me.reportFilters[index].title).val() + "')";
							}
							else if ($("#" + me.reportFilters[index].title).val() != "" && $("#" + me.reportFilters[index].title + "_1").val() != "" && $("#sel" + me.reportFilters[index].title).val() == 4) {
		...
		moduleColumnDataLoaded: function(me, activeId) {
			...
			if (me.moduleColumnDatas.length > 0) {
				for(var index = 0; index < me.moduleColumnDatas.length; index++) {
					...
					rowData += "<tr id='adhReportDataRow" + fin.reportUi.moduleColumnDatas[index].primeColumn + "' onclick=(fin.reportUi.getAdhReprotGridRowEdit(" + pkIndex + "," + index + "));>";	
		...
		populateOperatorDropDown: function(id) {
			...
			rowHtml += "<option title='=' value='1' selected>=</option>";
			...
			for (var index = 0; index < row[0].cells.length; index++) {
				...
				if (posTTable > 0) {
					...
					switch (me.columnType) {
						case 1: //Editable
							rowData += "<td class='gridColumn' align='left'>" + me.populateDropDown(argTypeTable, argName, row[0].cells[index].innerHTML, me.pkId) + "</td>";
							break;
					...
					if (me.isDate(row[0].cells[index].innerHTML))
