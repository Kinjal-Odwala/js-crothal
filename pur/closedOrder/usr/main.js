...
ii.Import( "fin.pur.closedOrder.usr.defs" );
...
ii.Class({
    Name: "fin.pur.closedOrder.UserInterface",
    Definition: {
		init: function() {
			var me = this;
			...
			me.authorizePath = "pur\\closedOrder";
		...
		configureCommunications: function() {
			...
			me.houseCodeJobStore = me.cache.register({
				...
				itemConstructor: fin.pur.closedOrder.HouseCodeJob,
				itemConstructorArgs: fin.pur.closedOrder.houseCodeJobArgs,
			...
			me.purchaseOrderDetailStore = me.cache.register({
				...
				itemConstructor: fin.pur.closedOrder.PurchaseOrderDetail,
				itemConstructorArgs: fin.pur.closedOrder.purchaseOrderDetailArgs,
		...
		houseCodeJobsLoaded: function() {
			...
			for (var index = 0; index < parent.fin.purMasterUi.houseCodeJobs.length; index++) {
				...
				me.houseCodeJobs.push(new fin.pur.closedOrder.HouseCodeJob(job.id, job.jobNumber, job.jobTitle));
			}
...
function main() {
	fin.closedOrderUi = new fin.pur.closedOrder.UserInterface();
	fin.closedOrderUi.resize();
}