...
Bud.page.BudgetDetailsPage = WebLight.extend(Bud.page.BudgetPage, {
        ...
        var grid = new WebLight.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel({
                ...
                columns: [
                    ...
                    { header: '&nbsp;<br />Percent Of<br />Billing' }
                ]
            })
        });

        this.addChildControl(Ext.create({
            xtype: 'box', html: '<h2 class="page-title">Budget Details</h2>'
        }), 'container');
        this.addChildControl(grid, 'container');
        this.addChildControl(panelVI = Ext.create({
            style: 'margin:10px 0', xtype: 'panel', border: false, items: [
                { border: false, html: '',
                    buttons: [
                    { ctCls: 'ux-button-1', text: 'Add Account Code', handler: function () { } },
    ...
    dataBind: function () {
        this.arrayStore.loadData(Bdg.store.Detail);
        Bud.page.BudgetDetailsPage.superclass.createChildControls.call(this);
    }
});
