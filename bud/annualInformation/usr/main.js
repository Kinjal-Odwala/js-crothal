...
ii.Class({
    ...
    Definition: {
        ...
        defineFormControls: function fin_bud_annualInformation_UserInterface_defineFormControls() {
            ...
            var rowNumber = -1;
            me.accountGrid.addColumn("assigned", "assigned", "", "Checked means assigned to annual Information", 30, function () {
                rowNumber += 1;
        ...
        yearsLoaded: function (me, activeId) {
            ...
            $('#AnchorCloneMiddle > span').html('&nbsp;&nbsp; Clone to Year ' + me.fiscalYear.data[0].name + '&nbsp;&nbsp');
            $('#AnchorClone').hide();
        },

        discardYearChange: false,

        yearChanged: function () {
            var me = this;

            if (me.discardYearChange) {
                me.discardYearChange = false;
                return;
            }
        ...
        actionCloneItem: function () {
            ...
            var year = me.fiscalYear.data[0].name;
            var priorYear = year - 1;
            me.discardYearChange = true;
            me.annualInformationId = 0;

            me.startDate.setValue(me.annualInformations[0].startDate.replace(/20\d{2}$/gi, priorYear));
            me.cutOffDate.setValue(me.annualInformations[0].cutOffDate.replace(/20\d{2}$/gi, year));
            me.fiscalYear.select(0, me.fiscalYear.focused);
            me.fiscalYearId = me.fiscalYear.data[me.fiscalYear.indexSelected].id;
            me.announcement.value = '';
        },
