...
var GetCurrentFscPeriodByToday = function () {
    ...
    window.__currentFscPeriod = FscPeriod;
...
var loadMopPeriodItems = function (hcmHouseCode, hcmJob, fscYear, callback) {
    ...
    var innerCallback = function (data) {
        ...
        $('item', $(data)).each(function (index, item) {
            ...
            var isRevenueAccount = mopAccountItem.CategoryId == 45;
            ...
            mopCategoryItem['Budget' + periodNumber] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.budget : dataItem.budget;
            ...
            mopCategoryItem['Projection' + periodNumber] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.mop : dataItem.mop;
            ...
            mopCategoryItem['BudgetTotal'] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.budget : dataItem.budget;
            ...
            */
            ...
            if (currentPeriod.FscYear < fscYear || currentPeriod.FscYear == fscYear && (currentPeriodIndex <= periodNumber || currentPeriodIndex - 1 == periodNumber && currentDay < FiscalPeriodCloseDays)) {
                mopAccountItem['ProjectionTotal'] += dataItem.forecast;
                mopCategoryItem['ProjectionTotal'] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.forecast : dataItem.forecast;
                ...
                mopCategoryItem['ProjectionTotal'] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.wor : dataItem.wor;
                if (FscAccount.MopTotalType != 3)
            ...
            mopCategoryItem['RunRate' + periodNumber] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.runrate : dataItem.runrate;
            if (FscAccount.MopTotalType != 3)
            ...
            mopCategoryItem['DailyRunRate' + periodNumber] += !FscAccount.IsNegative && isRevenueAccount ? -dailyRunRate : dailyRunRate;
            if (FscAccount.MopTotalType != 3)
            ...
            mopCategoryItem['Forecast' + periodNumber] += !FscAccount.IsNegative && isRevenueAccount ? -dataItem.wor : dataItem.wor;
            if (FscAccount.MopTotalType != 3)
    ...
    var parse = function (xml) {
        ...
        $('item', $(xml)).each(function (index, item) {
            ...
            var row = {
				...
				servicePeriod: $item.attr('servicePeriod')
            };
...
var saveWorSummaryComment = function (hcmHouseCode, hcmJob, fscYear, fscPeriod, comment, callback) {
 ...
 var $mop_templates = $(window.__bt__626c0767[2]);
...
MopDataGrid = function () {
    ...
    var $templates = $(window.__bt__626c0767[0]);
    ...
    var MopFilter = bine.extend(bine.DataView, {
        ...
        createFields: function () {
            ...
            var onhhcBeforeSelect = function () {
                if (fin.bud.modified()) {
                    HcmHouseCodeStore.clearFilter(false);
                    dv.refresh('hcmHouseCode');
                    return false;
                }
                else {
                    return true;
            ...
            var onBeforeSelect = function () { return !fin.bud.modified(); }
    ...
    var MopFooter = bine.extend(bine.Control, {
        ...
        onDomReady: function () {
            ...
            var cancelButton = new bine.Button({ text: 'Cancel' });
            me.append(cancelButton, 'cancelButton');
    ...
    bud.page.MOP = bine.extend(bine.Control, {
        ...
        createControls: function () {
            ...
            me.filter.on('fscyearchange', function (fscYear, fscPeriods) {
                ...
                var currentPeriodInSelectedYear = false;

                if (currentPeriod)
                    currentPeriodInSelectedYear = bine.query(fscPeriods).where(function (p) { return p.Id == currentPeriod.Id; }).first() != null;

                if (currentPeriodInSelectedYear) {
            ...
            me.columnSwitch.on('apply', function (name) {
                ...
                me.mopList.refreshUi();
                me.refreshScrolls();
                document.getElementById(me.id('mop-grid-wrapper')).scrollLeft = 0;
                me.resizeUi();
            });
            ...
            me.filter.on('load', function (filter) {
                ...
                isBudgetStarted(filter.hcmHouseCode, filter.hcmJob, filter.fscYear, function (started, exported) {
                    if (started) {
                        if (exported)
                            me.loadMopDetails();
                        else
                            alert('The budget has not been exported');
                    }
                    else {
        ...
        initMopDataControllerHeader: function () {
            ...
            me.controllerheader.on('updatefuture', function () {
                ...
                var periodCount = me.columnSwitch.fscPeriods.length;
            

                $.each(me.mopData, function (index, item) {
                    if (item.UpdateFuture) {

                        for (var i = periodIndex + 1; i <= periodCount; i++) {
        ...
        projectionUpdated: function (fscAccount, period, newValue) {
            ...
            var isRevenueAccount = accountItem.CategoryId == 45;

            if (FscAccount.MopTotalType == 1 && categoryItem) {
                var categoryVariance = !FscAccount.IsNegative && isRevenueAccount ? -variance : variance;
                categoryItem['Projection' + i] = categoryItem['Projection' + i] + categoryVariance;
                categoryItem['Variance' + i] = categoryItem['Variance' + i] + categoryVariance;
                categoryItem['ProjectionTotal'] = categoryItem['ProjectionTotal'] + categoryVariance;
            }
        ...
        resizeUi: function () {
            ...
            me.$('#mop-grid-wrapper').css('height', winHeight - 268);
